...
class Request:
    def __init__(self, req_type: str, action: str = None, resource: str = None, data: dict = None):
        """
        Initializes a Request object.

        :param req_type: The type of request, e.g., 'GET' or 'POST'.
        :param action: (LEGACY) The specific action to be taken, e.g., 'start', 'createWorkpiece'. 
                       For pure API v2, use only resource URLs.
        :param resource: The resource URL path, e.g., '/api/v2/stats/all', '/api/v2/stats/pumps/5'.
        :param data: The data related to the request (required for POST, optional for GET).
        """
        self.req_type = req_type  # 'GET' or 'POST'
        self.action = action      # LEGACY: Specific action - should be None for pure API v2
        self.resource = resource if resource else ""  # API v2: Full URL path like '/api/v2/stats/all'
        self.data = data if data is not None else {}  # Request body data
