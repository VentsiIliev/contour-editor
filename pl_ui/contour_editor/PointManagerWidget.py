...
from pl_ui.contour_editor.SegmentSettingsWidget import SegmentSettingsWidget
...
from pl_ui.contour_editor.LayerButtonsWidget import LayerButtonsWidget
from pl_ui.contour_editor.SegmentButtonsAndComboWidget import SegmentButtonsAndComboWidget
...
class PointManagerWidget(QWidget):
    ...
    def _add_points_for_segment(self, seg_index, segment):
        ...
        for i, pt in enumerate(segment.points):
            coords = f"({pt.x():.1f}, {pt.y():.1f})" if isinstance(pt, QPointF) else "N/A"
            self._add_point_item(f"P{i}", coords, seg_index, i, 'anchor')

        # Add control points
        for i, ctrl in enumerate(segment.controls):
            coords = f"({ctrl.x():.1f}, {ctrl.y():.1f})" if isinstance(ctrl, QPointF) else "N/A"
